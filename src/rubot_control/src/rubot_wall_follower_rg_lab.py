...
def clbk_laser(msg):
    ...
    bright_max = int(70 * scanRangesLengthCorrectionFactor)
    right_min = int(70 * scanRangesLengthCorrectionFactor)
    right_max = int(140 * scanRangesLengthCorrectionFactor)
    fright_min = int(140 * scanRangesLengthCorrectionFactor)
...
def take_action(regions):
    ...
    if regions['front'] > d and regions['fright'] > 2*d and regions['right'] > 2*d and regions['bright'] > 2*d:
        ...
        state_description = 'case 6 - unknown'
        linear_x = vx/2
        angular_z = -wz
